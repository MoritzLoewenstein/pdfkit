...
import { stringToUint8Array, isUint8Array } from 'uint8array-extras';
import PDFObject from './object';
import PDFReference from './reference';
import PDFPage from './page';
import PDFNameTree from './name_tree';
import PDFSecurity from './security_2';
...
import MiniEmitter from './mini_emitter';

class PDFDocument extends MiniEmitter {
  constructor(options = {}) {
    super();
    this.options = options;

    // Web ReadableStream setup
    this._controller = null;
    this._pendingChunks = [];
    this._endPromise = Promise.withResolvers();
    this._readable = new ReadableStream({
      start: (controller) => {
        this._controller = controller;
        if (this._pendingChunks.length) {
          for (const chunk of this._pendingChunks) controller.enqueue(chunk);
    ...
    this.initFonts(
      options.fonts?.name,
      options.fonts?.data,
      options.fonts?.family,
      options.fonts?.defaultSize,
    );
    this.initText();
    //this.initImages();
    this.initOutline();
  ...
  pipe(dest) {
    ...
    if (typeof dest.write !== 'function' || typeof dest.end !== 'function') {
      throw new Error('dest is not a valid nodejs writable stream');
    }
  ...
  push(data) {
    ...
    // emit Node-like 'data' event for compatibility
    this.emit('data', data);
  }
